...
def custom_auth_common_checks_warning(
    *,
    ...
    if not custom_auth_configured or run_common_checks:
        return None
    return (
        "custom_auth is configured but 'custom_auth_run_common_checks' is not set, so "
        "budgets, model-access allowlists, and per-model rate limits configured on your "
        "DB team/project records will NOT be enforced for custom-auth requests. "
        "Rate limits you set directly on the returned UserAPIKeyAuth still apply. "
        "Set 'general_settings.custom_auth_run_common_checks: true' to enforce them."
    )
